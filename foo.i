...
# 153 "/usr/include/i386-linux-gnu/bits/siginfo.h" 3 4
enum
{
  SI_ASYNCNL = -60,
...
# 307 "/usr/include/i386-linux-gnu/bits/siginfo.h" 3 4
...
void main(int argc, char *argv[])
{
 ...
 while ((opt = getopt(argc, argv, "s:p:i:")) != -1) {
  switch (opt) {
   ...
   case 'i': interval = atoi(optarg);
    break;
   default:
    printf("usage:""test_rate_server"" -s <size of date, default 1(M)>\n");
    exit(1);
  }
 }
 ...
 addrin.sin_addr.s_addr = htonl(((in_addr_t) 0x00000000));
# 81 "test_rate_server.c"
 sock_lis = socket(2, SOCK_STREAM, 0);
 ...
 reuse = 1;
 setsockopt(sock_lis, 1, 2, (const void *)&reuse, sizeof(reuse));
 reuse = 1;
 setsockopt(sock_lis, 1, 15, (const void *)&reuse, sizeof(reuse));
 ...
 while (1) {
  ...
  while (index++ < loop) {
   ...
   if (writed == 1024*interval) {
    ...
    printf("we have send %d M data, and speed is %.3fMB/s\n", interval, (writed*1024.0)/(result.tv_sec*1000 + (result.tv_usec/1000))/1000.0);
    gettimeofday(&once_start, 0);
    writed = 0;
   }
  }
  ...
  printf("we have send %f M data, ", total_writed/1024.0);
